...
IoC는 Inversion of Control의 약자이다. 우리나라 말로 직역해 보면 "역제어"라고 할 수 있다. 제어의 역전 현상이 무엇인지 보면 제어권이 업제 업제 객체에 있었다.   
그러나, Servlet, EJB 등을 사용하는 경우 Servlet Container, EJB Container에게 제어권이 넘어가서 객체의 생명주기(Life Cycle)를 Container들이 전담하게 된다.   
이처를 IoC에서 이야기하는 제어권의 역전이 란 객체의 생성에서부터 생명주기의 관리까지 모든 객체에 대한 제어권이 바뀌었다는 것을 의미한다.
...
#### 설정 예시
다음은 XML 기반으로 빈을 정의하고 의존성을 주입하는 간단한 예시이다.
...
<beans xmlns="http://www.springframework.org/schema/beans"
    ...
    <bean id="helloService" class="egovframework.example.service.impl.HelloServiceImpl">
        <property name="message" value="Hello, eGovFrame!" />
    </bean>
</beans>
```

다음은 ApplicationContext를 통해 컨테이너로부터 빈을 조회하여 사용하는 예시이다.
